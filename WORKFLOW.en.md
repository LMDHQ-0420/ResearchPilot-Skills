...
In Phases A and B, **every output from Claude begins with a "confirmed content card"**, wrapped in two lines of equal signs, so you can always see the currently locked consensus:
...
The card holds only content **you have confirmed**; candidate directions and intermediate search lists never enter it — only papers you explicitly name as references are listed. The card stays in sync with `docs/user_requirements.md` in real time.
