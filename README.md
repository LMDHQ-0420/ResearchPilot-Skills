...
Claude 通过四轮迭代交互完成深度调研，产出 `idea_report.md` Part 1。

**问题域锚定**：初步检索 10–15 篇代表性论文，从三个维度汇报（该方向主要解决什么问题、主流方法大类、近两年活跃子方向），用户确认问题域范围和感兴趣方向。

**文献深度覆盖**：针对确认范围下载并精读论文，对每个方法大类输出"能解决什么 / 解决不了什么"的结构化分析，每个 gap 必须有论文段落作为证据；用户可补充论文或挑战 gap 判断，可多次迭代直到覆盖充分。

**RQ 提炼与验证**：从确认的 gap 推导 3–5 个候选 RQ，每个 RQ 标注对应 gap、新颖性核查结果（专项搜索）、可回答性评估；用户选定主 RQ 和次 RQ。

**必要性论证**：针对选定 RQ 撰写必要性论证（应用必要性 / 理论必要性 / 时机必要性），分点回答，每点附论文支撑；用户确认论证成立后，汇编生成 Part 1。

---

### 阶段 B：Idea 深化

Claude 在 Part 1 确认后，单独生成 `idea_report.md` Part 2。

**Part 2 — Idea 设计**：Introduction、Related Works（含研究空白分析）、Method（整体框架 3.1 → 通俗流程详解 3.2 → 理论推导 3.3+）。

**References**：MLA 格式，Part 1+2 的引用统一汇总，每条附主要工作和引用原因；所有引用通过 `web_search` 核实，无法确认的标注 `[待核实]`。
...
**约束收集**：询问 GPU 型号和训练时长上限，写入 `user_requirements.md`。

**Baseline 精读**：向用户展示计划精读的 baseline 论文和 GitHub 列表（含每项的精读目的），用户确认后逐一读取；从论文和代码中提取各 baseline 使用的数据集、划分方式、实验设计、对比模型、评估指标和关键超参数，汇总为 Part 3 第 0 节。

**领域惯例归纳**：综合精读结果，归纳各 baseline 共同使用的标准 benchmark、标准评估指标、消融设计惯例和结果汇报规范，作为实验设计的参考基准。

**可行性核实**：确认数据集链接可访问、Baseline 代码仓库可访问、GPU 显存满足要求；任一项不通过则暂停并告知用户。

**实验设计**（顶会标准工作量）：主实验 3–5 个数据集 + 5–8 个 baseline；消融实验系统覆盖所有创新模块（3–6 个变体）；附加实验 2–3 类（泛化性 / 效率 / 鲁棒性 / 可视化）；所有实验报告均值 ± 标准差，至少 3 次随机种子。每个实验注明：目的、数据集划分及理由、评估指标含义、预期效果、参与评测的模型。

---

### 阶段 D：实现设计

Claude 生成 `implementation.md`，作为阶段 E 编码的精确指南；每次生成或修改后自动执行三项校验（实验覆盖、逻辑一致性、完整性）。

**强 Baseline 路径**（基于已有开源项目）：git clone 原始项目 → 扫描现有结构 → 生成改写方案；每个需修改的函数说明原来做什么、改为做什么（文字步骤）、参数 / 返回值变化。

**从头构建路径**：完整目录树 + 每个文件职责；独立数据流章节（原始文件 → 解析 → 划分 → 标准化 → 模型输入 tensor，含 shape）；每个函数说明签名、参数含义、返回值语义、实现逻辑。

**数据准备**：`implementation.md` 确认后，Claude 输出数据下载指令，等待用户完成数据准备后进入阶段 E。
...
**模块校验**：每完成一个模块后与 `implementation.md` 做一次一致性校验（函数签名、tensor shape、评估指标）。

**错误处理**：发现 `implementation.md` 有误时停止 → 报告问题和建议修改方案 → 等用户确认 → 先改 `implementation.md` → 再改代码。

**依赖规则**：`requirements.txt` 只写库名，不写版本号，不含 `torch` / `torchvision` / `torchaudio`。
