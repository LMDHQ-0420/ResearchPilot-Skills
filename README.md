...
ResearchPilot-Skills 是一个 Claude Code Skill，将完整的学术研究流程自动化：从方向探索、文献检索、Idea 深化、实验设计、代码实现，到论文撰写。全程通过对话自然推进，Claude 在每个关键节点主动询问是否继续，无需记忆切换命令。
...
- **有效性优先 + 实现校验**：实验设计第一目的是严格证明 idea 有效性，不为资源削减实验（资源仅在设计后作预估）；设计前只核实数据/代码可获取性；实现指南自动校验实验覆盖、逻辑一致性、完整性。
- **论文撰写带版本与批注**：先确认论文结构再写初稿；正文留空白 `>` 供你逐处批注，Claude 读批注改稿；每改一版独立存档（`v{大}.{小}-{简述}`），图表由 Python 脚本按论文格式生成。
...
应看到 7 个目录：`research[START]`、`research[A]-exploration`、`research[B]-idea`、`research[C]-experiment`、`research[D]-implementation`、`research[E]-coding`、`research[F]-paper`。
...
> `/research 描述` 是 `/research[START] 描述` 的向后兼容别名，效果相同。
...
| **C** | 实验设计 | 精读 baseline 论文与代码，归纳领域惯例，先与你确认实验大纲再展开完整方案并给出资源预估 | `idea_report.md` Part 3 |
| **D** | 实现设计 | 生成精确到函数的编码指南，自动校验覆盖/一致性/完整性 | `implementation.md` |
| **E** | 编码 | 按指南逐文件实现，同步维护开发日志，逐模块校验，完工后主动代码审查 | 代码 + `dev_log.md` |
| **F** | 论文撰写 | 先确认论文结构，生成初稿，按你的批注逐版改稿（每版独立存档），引导生成图表 | `docs/manuscripts/` 论文 |
...
docs/
  ...
  implementation.md     # 逐文件、逐函数的实现指南（开头含目录树+文件功能表，含数据流、校验记录）
  dev_log.md            # 编码进度与决策日志
  user_requirements.md  # Claude 通过对话收集的约束（方向偏好、RQ 约束、实现约束等）
  papers/               # 下载的论文 PDF 或摘要 TXT
  manuscripts/          # 阶段 F 论文，每改一版独立存档 v{大}.{小}-{简述}.md

code/
  README.md             # 项目内容、环境配置、详细运行命令（位置可选项目根或 code/）
  requirements.txt
  src/                  # 核心模型与训练代码
  scripts/              # 数据处理与实验脚本
  configs/              # 超参数配置
