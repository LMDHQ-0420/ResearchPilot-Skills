...
用户确认 implementation.md 后，**在开始编码前，逐项与用户确认以下 5 件事**（一次性提出，用户可逐条回应）：
...
每次生成或修改 implementation.md 后，必须执行以下校验，将问题列出并修复后再继续：
...
- `src/models/baseline/` 是否覆盖 Part 3 中所有参与对比的 baseline 模型
- data 目录结构是否与数据流描述一致
