...
阶段 A 和阶段 B 的**每一次输出**，开头都必须先输出"已确认内容卡片"，用两行等号包裹，让用户随时看到当前已锁定的共识。格式如下：

```
=====================================
**已确认内容**

**研究方向**：{已确认的研究方向一句话描述；尚未确认时填"待确认"}

**研究问题（RQ）**：
- RQ1（主）：{已确认的主 RQ；未确认填"待确认"}
- RQ2（次）：{已确认的次 RQ；未确认则省略此行}

**用户约束**：
- 方向约束：{用户对研究方向提出的约束；无则填"无"}
- RQ 约束：{用户对研究问题提出的约束；无则填"无"}

**明确参考论文**：{用户明确点名要参考的论文；无则省略整项}
=====================================
```

**卡片规则**：
- 卡片只放"已被用户确认"的内容，未确认的方向/RQ 候选不进卡片
- 卡片**不含**详细文献检索清单（那是过程性内容）；仅当用户明确点名某篇论文作为必读参考时，才列入"明确参考论文"
