...
1. 下载或克隆此仓库
2. 将 `code/skills/research-scout-zh/` 目录复制到 `~/.claude/plugins/marketplaces/research-scout-zh/`
3. 重启 Claude Code
...
```bash
# 模式 1 — 研究方向字符串
/research-scout-zh "电池健康状态预测"

# 模式 2 — 附带种子论文（PDF、名称或描述）
/research-scout-zh "电池 SOH" --papers paper1.pdf "BattFormer 2023"
...
2. 创建 `docs/user_requirements.md` 阶段 1 章节 → 你填写 → `/research-scout-zh confirm`
3. 双轨检索 15+ 篇论文（用户论文 + 自主 web 搜索）
4. 生成 3-5 个候选 idea，附新颖性检查与自我批判
5. 你选择一个：`/research-scout-zh pick 3`
6. Claude 深化选定 idea 并生成 `docs/idea_report.md` Part I
7. 尝试下载 PDF；若失败则提示

**阶段 1 审查节点** — Claude 在此停止。审查 `docs/idea_report.md` Part I，然后：
- `/research-scout-zh step2` — 确认并进入阶段 2
- `/research-scout-zh revise "修改意见"` — 带修改重新生成 Part I
...
- `/research-scout-zh step3` — 确认并进入阶段 3
- `/research-scout-zh revise "修改意见"` — 重新生成 Part II
...
3. 展示结构 → `/research-scout-zh confirm` 继续或 `/research-scout-zh revise-structure "意见"`
...
```bash
/research-scout-zh back-to-step2 "数据集无法访问"
# 归档 dev_log.md
# 标记 Part II 状态为 REVISING
# 运行 /research-scout-zh revise "意见" 更新 Part II
# 然后 /research-scout-zh step3 重新开始编码
```
...
| 命令 | 阶段 | 说明 |
|------|------|------|
| `/research-scout-zh "topic"` | 1 入口 | 提供研究方向启动 |
| `/research-scout-zh --papers <文件/名称>` | 1 入口 | 提供种子论文启动 |
| `/research-scout-zh`（自由描述）| 1 入口 | 从用户描述中提取 |
| `/research-scout-zh confirm` | 1, 3 | 确认论文、PDF、或结构 |
| `/research-scout-zh revise-paper {n} "修正"` | 1 | 修正第 n 条论文推断 |
| `/research-scout-zh skip-papers` | 1 | 跳过缺失的 PDF |
| `/research-scout-zh pick {n}` | 1 | 选择候选 idea |
| `/research-scout-zh step2` | 1→2 | 进入实验设计 |
| `/research-scout-zh step3` | 2→3 | 进入编码 |
| `/research-scout-zh revise "意见"` | 1, 2 | 重新生成当前阶段文档 |
| `/research-scout-zh revise-structure "意见"` | 3 | 调整项目结构 |
| `/research-scout-zh back-to-step2 "原因"` | 3 | 回溯到阶段 2 |
| `/research-scout-zh log-results` | 3c | 记录实验结果 |
...
1. 检查是否正确输入命令 `/research-scout-zh`（带 `-zh` 后缀）
2. 重启 Claude Code
3. 确认软链接路径正确：`ls -la ~/.claude/plugins/marketplaces/`

### 可以修改生成的文档格式吗？

可以。在 `user_requirements.md` 的"文档偏好设置"中声明，或直接修改 `skills/research-scout-zh/references/document-formats.md`。

### Phase 1 生成的 idea 不满意怎么办？

运行 `/research-scout-zh revise "更具体的方向指导"`，Claude 会重新生成候选 idea。

### 如何从失败的实验中恢复？

使用 `/research-scout-zh back-to-step2 "失败原因"`，修改实验设计后重新进入 Phase 3。
